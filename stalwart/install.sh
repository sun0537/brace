...
示例:
  $0                          # 安装到默认目录 /opt/stalwart
  $0 install /usr/local/stalwart  # 安装到指定目录
  $0 install --force-init     # 重新安装并重置配置
  $0 uninstall                # 完全卸载
    ...
    if [ $# -gt 0 ]; then
        case "$1" in
            ...
            --*)
                ...
                ;;
            *)
                # Could be directory, treat as install
                _command="install"
                ;;
        esac
    fi
...
do_install() {
    ...
    local _is_update=0
    if [ -f "$_dir/bin/stalwart" ]; then
        _is_update=1
        say "📦 Detected existing Stalwart installation, performing update..."
        
        # Stop service before update
        if [ "${_os}" = "linux" ]; then
            if systemctl is-active --quiet stalwart.service 2>/dev/null; then
                say "⏸️  Stopping Stalwart service..."
                systemctl stop stalwart.service
            fi
        elif [ "${_os}" = "macos" ]; then
            if launchctl list | grep -q stalwart.mail 2>/dev/null; then
                say "⏸️  Stopping Stalwart service..."
                launchctl stop system/stalwart.mail 2>/dev/null || true
            fi
        fi
    fi
    ...
    # Backup config if doing force-init on existing installation
    if [ $_force_init -eq 1 ] && [ -f "$_dir/etc/config.toml" ]; then
        local _backup="$_dir/etc/config.toml.backup.$(date +%Y%m%d_%H%M%S)"
        say "💾 备份现有配置到: $_backup"
        ensure cp "$_dir/etc/config.toml" "$_backup"
    fi
    ...
    # Run init only if config doesn't exist or force-init is specified
    if [ ! -f "$_dir/etc/config.toml" ] || [ $_force_init -eq 1 ]; then
        $_dir/bin/stalwart --init "$_dir"
    fi
    ...
    if [ -f "$_dir/etc/config.toml" ]; then
    ...
    if [ $_is_update -eq 0 ]; then
        ...
        say "✅ Update complete! Stalwart service has been restarted"
    fi
...
do_uninstall() {
    ...
    if [ "${_os}" = "linux" ]; then
        ...
        if [ -f /Library/LaunchAgents/stalwart.mail.plist ]; then
            say "⏹️  Stopping and disabling service..."
            launchctl stop system/stalwart.mail 2>/dev/null || true
            launchctl disable system/stalwart.mail 2>/dev/null || true
            launchctl unload /Library/LaunchAgents/stalwart.mail.plist 2>/dev/null || true
            say "🗑️  Removing LaunchAgent configuration file..."
    ...
    if [ -d "$_dir" ]; then
        say "🗑️  Removing installation directory: $_dir"
        rm -rf "$_dir"
    fi

    # Remove user and group
    if id -u ${_account} > /dev/null 2>&1; then
        say "🗑️  Removing system account: ${_account}"
        if [ "${_os}" = "macos" ]; then
            dscl /Local/Default -delete Users/_stalwart 2>/dev/null || true
            dscl /Local/Default -delete Groups/_stalwart 2>/dev/null || true
        else
            userdel ${_account} 2>/dev/null || true
            groupdel ${_account} 2>/dev/null || true
        fi
    fi

    say "✅ Stalwart has been completely uninstalled"
    return 0
}
...
EOF
    launchctl load /Library/LaunchAgents/stalwart.mail.plist 2>/dev/null || true
